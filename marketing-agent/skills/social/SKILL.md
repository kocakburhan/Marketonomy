...
- If a social media visual is needed, do not stop at a brief; generate the actual visual.
...
- If the image generation tool is not active, do not write as if the visual was produced; save
  the prompt and write that production is pending in the `Gorsel Dosyasi` field.
- Always include the generated visual file path or "production pending" note; never leave the
  visual deliverable unspecified.
