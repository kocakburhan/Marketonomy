...
- If a marketing image is needed, do not stop at a brief; generate the actual image.
...
- Generate the visual using the active image generation flow within Codex.
- After production, add the prompt, production note, and image file path to the deliverable.
- If the image generation tool is not active, do not write as if the image was produced; save
  the prompt and write that production is pending.
